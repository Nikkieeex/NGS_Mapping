...
*opdracht 4* In deze opdracht moeten we nog een visualisatie maken gebaseerd op de BED-data maar nu gebruiken we de GRange object.
...
BAM-bestand laden en pileup voor één gen en Rsamtools gebruiken En bam en bai files nog op logische plek zetten
...
per gen rapporteren: aantal basen gemiddelde dekking aantal posities met lange dekking en percentage
...
Het resultaat van deze tool zou uiteindelijk een nieuwe file worden (vcf), elke lijn beschrijft een single variant. In galaxy kan je makkelijk zien hoeveel varianten er zijn. Noteer de 17 comment lines aan het begin van de file. *Opdracht 8:* Omdat we met patienten data werken gaan we ook volgends de richtlijnen werken van UMCG. Dit houdt in dat sequenties met een minimale waarde van 30% behouden word. Alles wat hier niet aan voldoet word eruit gefiltered.
...
De volgende stap is om alle varianten te krijgen van een exon. Dit moet met de findOverlap functie: <https://www.rdocumentation.org/packages/GenomicAlignments/versions/1.8.4/topics/findOverlaps-methods>
...
*opdracht 9* Voor deze opdracht 2 simpele visualizaties:
...
##10-10-2025 Vandaag ga ik verder met de bovenstaande stap annotie met SNpEff
...
| dbNSFP\_\* | dbNSFP | Various: Predictive scores, allele counts, conservation, etc. |
...
Daarna de tool runnen met CHROM,POS,REF,ALT,dbNSFP_SIFT_pred,dbNSFP_SIFT_score,dbNSFP_Polyphen2_HDIV_pred,dbNSFP_Polyphen2_HDIV_score,dbNSFP_MutationAssessor_pred,dbNSFP_MutationAssessor_score,dbNSFP_MetaSVM_pred,dbNSFP_MetaSVM_score,dbNSFP_CADD_phred,dbNSFP_REVEL_score,dbNSFP_GERP\_\_\_RS,dbNSFP_phastCons100way_vertebrate (extract_tool2.tabular)
...
filtered_df_unique <- filtered_df[!duplicated(filtered_df[, c("CHROM", "POS", "REF", "ALT")]), ]
# Sorteer op impact_score en kies top 10
top10_df <- filtered_df_unique[order(-filtered_df_unique$impact_score), ][1:10, ]
...
Nu hebben we maar een top 4, ik denk dat dit komt omdat er te streng gefilterd is.

## 14-10-2025

Vandaag ga ik bezig met de vragen verder beantwoorden.

What is the effect of the variant? Het effect van de variant is missense, dit houdt in dat de DNA-verandering resulteert in enkele aminozuurveranderingen in de eiwitsequentie. Dit kan het functie van de eiwit veranderen maar dit hoeft niet altijd schadelijk te zijn. De impact level is moderate en dit geeft mogelijk functionele veranderingen in het eiwit aan, dit hoeft niet schadelijk of erg ernstig te zijn. De CLNSIG (klinische betekenis) geeft aan dat dit benign (goedaardig) is, deze varianten zijn niet geasioceerd met ziekten in klinische bases. Hoewel deze varianten als goedaardig worden aangeduid, zijn ze geannoteerd met dilated cardiomyopathy.

<https://www.medrxiv.org/content/10.1101/2025.07.17.25331109v1>

Is it validated? Is het gevalideerd?​ Alle varianten in de top zijn goedaardig, beschoud als niet ziekteveroorzakend.​ De genen wat betrokken zijn​ TTN, vaak betrokken bij cardiomyopathieeën veel varianten zijn goedaardig vanwege de grootte envariab iliteit van het gen.​ DMD, vaak betrokken bij Duchenne en Becker spierdystrofie maar ook veel van deze varianten zijnpatho geen.​ Niet klinisch gevalideerd als pathogeen ondanks de assisocatie, vanwege goedaardig.

https://www.cell.com/ajhg/abstract/S0002-9297(25)00324-6

Are there scientific studies supporting the evidence?Zijn er wetenschappelijke studies die het bewijs ondersteunen? Er zijn een aantal studies A comprehensive framework for the interpretation of TTN missense variants | Sciety TTN is een groot variabel gen waardoor interpretatie van missense varianten lastig kunnen zijn. De missense varianten worden aangetroffen bij "gezonde mensen en worden als goedaardig geclassifiseerd. Studie stelt een kader voor om pathogene van niet pathogene TTN-varianten te onderscheiden met populatiefruquentie en context van proteïnedomeinen.

What does the score indicate?
De score is uitgerekend met punten, high 2 punten moderate 1 en de rest is 0 punten. 

What else can you find about the variant?
Grootste genoom van mensen.
